...
import { MdDelete, MdFastfood, MdFoodBank } from "react-icons/md";
...
import { generateItemtId, generateUniqueId } from "../utils/generateUniqueId";
import LoadingScreen from "../components/LoadingScreen";

const addcustomer = () => {
  const router = useRouter();
  const { tableid, tablename } = router.query;

  const currentDate = new Date();
  const hour = currentDate.getHours();
  const minuits = currentDate.getMinutes();

  const INITIALSTATE = {
    billingID: generateUniqueId(),
    billing_date: new Date().toISOString().substring(0, 10),
    billing_time: `${hour}:${minuits < 10 ? "0" : ""}${minuits}`, // Zero-pad minutes
    customer_table: tablename ? tablename : "",
    table_id: tableid,
    customer_name: "",
    customer_contact: "",
  ...
  const {
    ...
    queryFn: () =>
      axios.get(`/api/items/getallitem?search=${searchQuery}`).then((res) => {
        return res.data;
      }),
  });
  ...
  const { data: TableRecords } = useQuery({
    ...
    queryFn: () =>
      axios.get(`/api/tables/gettables`).then((res) => {
        return res.data;
      }),
  });
  ...
  const removeItemTest = (itemID, itemId) => {
    const updatedOrdered = FormData.ordered.filter(
  ...
  const { data: CategoryRecord } = useQuery({
    ...
    queryFn: () =>
      axios.get(`/api/items/getcategory`).then((res) => {
        return res.data;
      }),
  });
  ...
  const DecreaseQuantity = (itemID, itemId) => {
    ...
    const index = updatedOrderedItems.findIndex(
      (item) => item.itemid === itemId
    );
    if (index !== -1) {
      if (updatedOrderedItems[index].qty > 1) {
        ...
        setFormData({
          ...FormData,
          ordered: updatedOrderedItems,
        });
      } else {
        // Remove item if quantity is 1
        removeItemTest(itemID, itemId);
      }
  ...
  const handleItemClick = (itemName, itemid) => {
    ...
    const existingItemIndex = FormData.ordered.findIndex(
      (item) => item.itemid === itemid
    );
  ...
  return (
    <div>
      ...
      <div className="px-5 mb-10 mt-24">
        {ShowCustomer && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 backdrop-blur-sm z-50 p-4 sm:p-6">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md border-2 border-orange-100 sm:max-w-lg md:max-w-xl">
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="form-control">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="input input-bordered w-full pl-12 bg-orange-50 border-orange-200 focus:border-orange-400 text-sm sm:text-base py-2 sm:py-3"
                    />
                    <FaSignature className="absolute left-3 top-3 text-orange-400 w-5 h-5" />
                  </div>
                </div>
                ...
                <div className="form-control">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="input input-bordered w-full pl-12 bg-orange-50 border-orange-200 focus:border-orange-400 text-sm sm:text-base py-2 sm:py-3"
                    />
                    <FaMobileAlt className="absolute left-3 top-3 text-orange-400 w-5 h-5" />
                  </div>
                </div>
                ...
                <div className="form-control">
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={HandleChange}
                      className="select select-bordered w-full pl-12 bg-orange-50 border-orange-200 focus:border-orange-400 text-sm sm:text-base py-2 sm:py-3"
                    >
                      <option value="">Select table</option>
                      <option value="PARCEL" className="text-orange-600">🚚 PARCEL</option>
                      {TableRecords?.map((data) => (
                        <option value={data.table_name} key={data.id}>
                          🪑 {data.table_name}
                        </option>
                      ))}
                    </select>
                    <FaMapMarkerAlt className="absolute left-3 top-3 text-orange-400 w-5 h-5" />
                  </div>
                </div>
                ...
                <div className="form-control">
                  ...
                  <div className="relative">
                    ...
                    <FaTruck className="absolute left-3 top-3 text-orange-400 w-5 h-5" />
                  </div>
                </div>
                ...
                <button
                  onClick={() => setShowCustomer(false)}
                  className="btn w-full bg-gradient-to-r from-orange-500 to-red-500 text-white hover:from-orange-600 hover:to-red-600 py-3 sm:py-4 text-sm sm:text-base flex items-center justify-center gap-2"
                >
                  {Loading ? (
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-lg border border-orange-100 overflow-hidden flex flex-col h-[100vh]">
            ...
            <div className="bg-gradient-to-r from-red-500 to-orange-500 p-4 flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-2 text-white">
                ...
                <h1 className="lg:text-lg  text-sm font-bold font-cursive">Delicious Menu</h1>
                <IoFastFoodSharp className="w-8 h-8" />
              </div>
            ...
            rounded-full gap-2 min-w-[120px] hover:scale-105 transition-transform`}
              ...
              rounded-full gap-2 min-w-[80px]  hover:scale-105 transition-transform`}
                 ...
                 border border-orange-100 cursor-pointer group"
                      ...
                      >
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <h3 className="lg:text-lg text-sm font-semibold text-gray-800 group-hover:text-red-600">
                            {item.item_name}
                          </h3>
                        </div>
                        <div className="flex items-center justify-between">
                          <div className="badge badge-outline text-sm  border-red-200 text-red-600">
                            {item.item_cate}
                          </div>
